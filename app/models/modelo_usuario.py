    ...
    def login(self, usuario):
        ...
        try:
            ...
            with connection.cursor() as cursor: 
                ...
                cursor.execute("SELECT id, username, password, email FROM usuarios WHERE username = %s", (usuario.username,))
...
#Este metodo sirve para que se pueda usar el id como inicio de sesion el id_jugador se recibe del metodo def load_user(id_jugador) que esta en el archivo app.py 
    ...
    def obtener_por_id(self,id):
        try:
            connection = get_connection()
            with connection.cursor() as cursor: 
                cursor.execute("SELECT id, username, password, email FROM usuarios WHERE id = %s",(id))#En el format se le pasa el atributo que recibe el metodo obtener_por_id  
